...
from core.units.info import UnitInfoMetaClass, unitlistpertype
...
class WarsBuildingInfo(AbilityPlaceObjectShared, UnitInfo, metaclass=BuildingInfoMetaClass):
    ...
    def PlaceObject(self):
        """ Places the building.
        For modes with limited constructions, the construction is limited when shift-queuing construction order
        """
        try:
        ...
        rules_info = getattr(gamerules(), 'info', None)
        unit_limits = getattr(rules_info, 'unit_limits', {})

        unitname = self.name
        limit = unit_limits.get(unitname, None)

        current_count = len(unitlistpertype.get(owner, {}).get(unitname, []))
        if limit is not None and current_count >= limit:
            self.RefundResources(owner, costs=self.resourcestaken)
            return None
    ...
    def RefundResources(self, owner=None, costs=None):
        if not owner:
            owner = self.GetOwnerNumber() if hasattr(self, 'GetOwnerNumber') else 0

        if not costs:
            costs = getattr(self, 'purchasecosts', None)

        if not costs:
            return

        for resource_list in costs:
            for rtype, amount in resource_list:
                if amount > 0:
                    GiveResources(owner, [(rtype, amount)])
