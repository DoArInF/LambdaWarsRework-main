...
"""
from srcbase import KeyValues, KeyValuesDumpAsDevMsg, KeyValuesToDictFromFile
import filesystem
...
def LoadFileIntoDictionaries(path, getfirstkey=True, default=None):
    if not filesystem.FileExists(path):
        return default
        
    data = KeyValuesToDictFromFile(path)
    if data is None:
        return default
